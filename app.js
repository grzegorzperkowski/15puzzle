(() => {
  ...
  function startTimer() {
    ...
    state.timerId = window.setInterval(renderTime, 200);
  }
  ...
  async function moveTile(index) {
    if (state.paused || state.solved || state.animating || elements.resetDialog.open || elements.successDialog.open) return;
  ...
  elements.board.addEventListener("keydown", (event) => {
    ...
    if (!event.repeat) moveEmptySpace(event.key);
  });
